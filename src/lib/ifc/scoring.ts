/**
 * IFC Readiness score calculation.
 *
 * Mirrors the logic in src/lib/scoring/index.ts exactly, operating on the
 * IFC requirements taxonomy instead of the EXIM taxonomy. Exports
 * `computeIfcReadiness` with the same signature as `computeReadiness`.
 */

import {
  IFC_REQUIREMENTS,
  IFC_LOI_CRITICAL_IDS as _IFC_LOI_CRITICAL_IDS,
} from "./requirements";
...
/**
 * Computes the IFC readiness score for a project given its current
 ...
 */
export function computeIfcReadiness(statuses: RequirementInput[]): ReadinessResult {
  ...
  const loiCriticalSet = new Set(_IFC_LOI_CRITICAL_IDS);

  for (const req of IFC_REQUIREMENTS) {
